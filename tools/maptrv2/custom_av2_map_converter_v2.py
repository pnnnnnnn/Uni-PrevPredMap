...
def create_av2_infos_mp(root_path,
    ...
    #     result = get_data_from_logid(log_id, loader, root_path, pc_range)
...
def get_data_from_logid(log_id, 
    ...
    print("effective: ", len(samples), ", discarded: ", discarded)
...
def extract_local_map(avm, e2g_translation, e2g_rotation, pc_range):
    ...
    dict_divider_type = {'divider_none': [LaneMarkType.NONE],
                         'divider_solid': [LaneMarkType.SOLID_WHITE, LaneMarkType.SOLID_YELLOW],
                         'divider_dashed': [LaneMarkType.DASHED_WHITE, LaneMarkType.DASHED_YELLOW],
                         'divider_double': [LaneMarkType.DOUBLE_SOLID_WHITE, LaneMarkType.DOUBLE_DASH_WHITE, \
                            LaneMarkType.DOUBLE_SOLID_YELLOW, LaneMarkType.DOUBLE_DASH_YELLOW, \
                            LaneMarkType.DASH_SOLID_WHITE, LaneMarkType.SOLID_DASH_WHITE, \
                            LaneMarkType.DASH_SOLID_YELLOW, LaneMarkType.SOLID_DASH_YELLOW],
                        }
    nearby_dividers = generate_nearby_dividers(avm, patch_box, patch_angle, dict_divider_type)
    ...
    for k, v in nearby_dividers.items():
        map_anno[k] = extract_local_divider(v, ego_SE3_city, patch_box, patch_angle, patch_size)
...
def generate_nearby_centerlines(avm, patch_box, patch_angle):
    ...
    all_paths = []
    for root in roots:
        # print('before root: ', root, ', leaves: ', leaves)
...
def generate_nearby_dividers(avm, patch_box, patch_angle, dict_divider_type):
...
#     nearby_ls_ids = []
    ...
    dict_nearby_dividers = {}
    for k, v in dict_divider_type.items():
        left_lane_dict = {}
        right_lane_dict = {}
        for key,value in divider_ls_dict.items():
            if value.left_neighbor_id is not None and value.left_mark_type in v:
            ...
            if value.right_neighbor_id is not None and value.right_mark_type in v:
        ...
        dict_nearby_dividers[k] = local_dividers

    return dict_nearby_dividers
