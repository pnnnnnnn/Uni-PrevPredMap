...
def timestamp_indexed(dict_gt_pkl):
    ...
    dict_gt_hdmap = {}
    for sample in dict_gt_pkl['samples']:
        ...
        for name, list_vec in sample['annotation'].items():
            if name in CLASS2LABEL_GLOBAL.keys():
                ...
                for vec in list_vec:
                    ...
                    vec_itpl = np.array([list(vec.interpolate(distance).coords) for distance in distances]).reshape(-1, 3)
                    dict_gt_hdmap[city_name][str(sample['timestamp'])][str(idx_class)].append(vec_itpl.tolist())
    return dict_gt_hdmap


dir_gt_pkl_train = "./data/argoverse2/av2_map_infos_train.pkl"
dict_gt_pkl_train = mmcv.load(dir_gt_pkl_train)
dict_gt_hdmap_train = timestamp_indexed(dict_gt_pkl_train)
dir_gt_pkl_val = "./data/argoverse2/av2_map_infos_val.pkl"
...
dir_trainval = "./data/argoverse2/av2_map_infos_trainval.json"
