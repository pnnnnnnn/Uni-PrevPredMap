    ...
    def forward(self,
        ...
        for lid, layer in enumerate(self.layers):
            if self.query_pos_embedding == 'instance':
                ...
                point_query_pos = None    # 也不会用到
                reference_points_input = reference_points[..., :2].unsqueeze(2)    # 好像不会用到
            ...
            if reg_branches is not None:
                ...
                # print("tmp: ", tmp.shape, "reference_points: ", reference_points.shape)
...
@TRANSFORMER_LAYER.register_module()
class ParallelDecoupledDetrTransformerDecoderLayer(BaseTransformerLayer):
    """Implements decoder layer in DETR transformer.
    Args:
        attn_cfgs (list[`mmcv.ConfigDict`] | list[dict] | dict )):
            Configs for self_attention or cross_attention, the order
    ...
    def __init__(self,
                 ...
                 num_group=6,
                 **kwargs):
        super(ParallelDecoupledDetrTransformerDecoderLayer, self).__init__(
            attn_cfgs=attn_cfgs,
            feedforward_channels=feedforward_channels,
            ffn_dropout=ffn_dropout,
            operation_order=operation_order,
    ...
    def forward(self,
        ...
        # print("attn_masks: ", kwargs['self_attn_mask'])
        # sys.exit()
        ...
        for layer in self.operation_order:
            if layer == 'self_attn':
                ...
                if attn_index == 0:
                    n_pts, n_batch, n_dim = query.shape
                    if self.training:
                        query = torch.cat(query.split(n_pts // group_nums, dim=0), dim=1)
                        if query_pos is not None:
                            query_pos = torch.cat(query_pos.split(n_pts // group_nums, dim=0), dim=1)
                    query = query.view(num_vec // group_nums, num_pts_per_vec, n_batch * group_nums, n_dim).flatten(1,2)
                    if query_pos is not None:
                        query_pos = query_pos.view(num_vec // group_nums, num_pts_per_vec, n_batch * group_nums, n_dim).flatten(1,2)
                    temp_key = temp_value = query
                    # print("query: ", query.shape)
                    # print(kwargs['self_attn_mask'].shape)
                    query = self.attentions[attn_index](
                        query,
                        temp_key,
                        temp_value,
                        ...
                        **kwargs)
                    # import ipdb;ipdb.set_trace()
                    query = query.view(num_vec // group_nums, num_pts_per_vec, n_batch * group_nums, n_dim).flatten(0,1)
                    if query_pos is not None:
                        query_pos = query_pos.view(num_vec // group_nums, num_pts_per_vec, n_batch * group_nums, n_dim).flatten(0,1)
                    if self.training:
                        query = torch.cat(query.split(n_batch, dim=1), dim=0)
                        if query_pos is not None:
                            query_pos = torch.cat(query_pos.split(n_batch, dim=1), dim=0)
                    attn_index += 1
                    identity = query
                else:
                    # import ipdb;ipdb.set_trace()
                    ...
                    if self.training:
                        ...
                        if query_pos is not None:
                            query_pos = torch.cat(query_pos.split(n_pts // group_nums, dim=0), dim=1)
                    query = query.view(num_vec // group_nums, num_pts_per_vec, n_batch * group_nums, n_dim).permute(1,0,2,3).contiguous().flatten(1,2)
                    if query_pos is not None:
                        query_pos = query_pos.view(num_vec // group_nums, num_pts_per_vec, n_batch * group_nums, n_dim).permute(1,0,2,3).contiguous().flatten(1,2)
                    temp_key = temp_value = query
                    query = self.attentions[attn_index](
                        query,
                        temp_key,
                        ...
                        **kwargs)
                    # import ipdb;ipdb.set_trace()
                    query = query.view(num_pts_per_vec, num_vec // group_nums, n_batch * group_nums, n_dim).permute(1,0,2,3).contiguous().flatten(0,1)
                    if query_pos is not None:
                        query_pos = query_pos.view(num_pts_per_vec, num_vec // group_nums, n_batch * group_nums, n_dim).permute(1,0,2,3).contiguous().flatten(0,1)
                    if self.training:
                        query = torch.cat(query.split(n_batch, dim=1), dim=0)
                        if query_pos is not None:
                            query_pos = torch.cat(query_pos.split(n_batch, dim=1), dim=0)
