...
class GlobalMap:
    def __init__(self, map_cfg):
        ...
        if dataset == 'av2':
            ...
            self.mix_min_lidar_loc = {'WDC': np.array([1664.20793519, 25.76974403]) - bev_radius,
                                        'MIA': np.array([-1086.92985063, -464.15366362]) - bev_radius,
                                        'PAO': np.array([-3050.01628955, -309.44287914]) - bev_radius,
                                        'PIT': np.array([695.66044791, -443.89844576]) - bev_radius,
                                        'ATX': np.array([589.98724063, -2581.61138577]) - bev_radius,
                                        'DTW': np.array([-6111.0784155, 628.12019426]) - bev_radius}
            self.mix_max_lidar_loc = {'WDC': np.array([6951.32050819, 4510.96637507]) + bev_radius,
                                        'MIA': np.array([6817.02338386, 4301.35442342]) + bev_radius,
                                        'PAO': np.array([1646.099298, 2988.66862304]) + bev_radius,
                                        'PIT': np.array([7445.46486881, 3314.83461676]) + bev_radius,
                                        'ATX': np.array([3923.01840213, -1161.67712224]) + bev_radius,
                                        'DTW': np.array([11126.80825267, 6215.31771939]) + bev_radius}
        else:
    ...
    def load_map(self, device):
        # print("self.load_map_path: ", self.load_map_path)
    ...
    def create_map(self, device, status):
        for city_name in self.city_list:
            ...
            city_map = torch.zeros((map_height_ceil, map_width_ceil, self.num_classes), dtype=self.map_type, device=device)
            if self.fuse_method == 'prob':
    ...
    def update_map(self, city_name, trans, raster, status):
        ...
        # print("raster: ", raster.shape)    # 100, 200, 4
        # print("new_map: ", new_map.shape, ", old_map: ", old_map.shape)
        # print("new_map: ", new_map[new_map != 0])
        # print("old_map: ", old_map[old_map != 0])
    ...
    def get_map(self, city_name, trans, status, device, timestamp=None):
        ...
        # print("local_map: ", local_map.shape, self.global_map_dict[city_name].shape)
    ...
    def fuse_map(self, new_map, old_map):
        if self.fuse_method == 'all':
            ...
            new_map = new_map * self.update_value + 100    # 30, [100, 130]
